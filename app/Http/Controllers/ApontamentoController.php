...
class ApontamentoController extends Controller
{
    ...
    {
        $data = Apontamento::orderBy('created_at', 'desc')->paginate(env("PAGINACAO"));

        // ─── KPI Stats ───
        $stats = [
            'total_apontamentos' => Apontamento::count(),
            'total_quantidade' => Apontamento::sum('quantidade'),
            'total_hoje' => Apontamento::whereDate('created_at', today())->count(),
        ];

        return view('estoque.apontamento', compact('data', 'stats'));
    }

...
class CteOsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Query base reutilizada nos stats e na listagem
        $baseQuery = CteOs::where('empresa_id', request()->empresa_id)
        ...
        // KPI cards — sempre sem filtro de estado para mostrar totais gerais
        $statsQuery = CteOs::where('empresa_id', request()->empresa_id)
        ->when(!empty($start_date), function ($query) use ($start_date) {
            return $query->whereDate('created_at', '>=', $start_date);
        })
        ->when(!empty($end_date), function ($query) use ($end_date) {
        ...
        $stats = [
            ...
            'valor'      => (clone $statsQuery)->where('estado_emissao', 'aprovado')->sum('valor_transporte'),
        ];

        $data = (clone $baseQuery)->orderBy('created_at', 'desc')->paginate(env("PAGINACAO"));

        return view('cte_os.index', compact('data', 'stats'));
    }

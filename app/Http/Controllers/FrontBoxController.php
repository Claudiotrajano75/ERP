...
class FrontBoxController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // ─── KPI Stats ───
        $statsQuery = Nfce::where('empresa_id', request()->empresa_id)
            ->when(!empty($start_date), function ($query) use ($start_date) {
                return $query->whereDate('created_at', '>=', $start_date);
            })
            ->when(!empty($end_date), function ($query) use ($end_date) {
        ...
        return view('front_box.index', compact('data', 'contigencia', 'stats'));
    }
    ...
    public function imprimirNaoFiscal($id)
    {
        ...
        // Instancia o Dompdf nativo
        $options = new \Dompdf\Options();
        $options->set('isRemoteEnabled', true); // Permite carregar imagens (logo/barcode)
        $dompdf = new \Dompdf\Dompdf($options);
        
        
        $dompdf->loadHtml($html);
        
        // Define papel (80mm de largura)
        // Altura dinâmica: Base (~300) + (Itens * ~25) - Reduzido para evitar rolagem
        $altura = 300 + (count($item->itens) * 25);
        
        // 1 pt = 0.352778 mm => 80mm = ~226.77 pt
        ...
        // Limpa qualquer saída anterior e encerra o buffer
        if (ob_get_level()) {
            ob_end_clean();
        }

        header('Content-Type: application/pdf');
        header('Content-Disposition: inline; filename="CUPOM_NAO_FISCAL.pdf"');

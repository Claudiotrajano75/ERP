...
class ProdutoController extends Controller
{
    ...
    public function __construct(UploadUtil $util, MercadoLivreUtil $utilMercadoLivre,
        ...
        ProdutoImagemService $produtoImagemService) {    $this->produtoImagemService = $produtoImagemService;
    $this->util = $util;
    ...
    {   
        ...
        $semEstoque = Produto::where('empresa_id', request()->empresa_id)
            ->whereHas('estoque', function($q) {
                $q->where('quantidade', '<=', 0);
            })->count();
            
        $valorEstoque = 0;
        $produtosComEstoque = Produto::where('empresa_id', request()->empresa_id)
            ->with(['estoque'])
            ->get();
        foreach($produtosComEstoque as $prod) {
            $qtd = $prod->estoque ? $prod->estoque->quantidade : 0;
            $valorEstoque += ($qtd * $prod->valor_venda);
        }
        ...
        return view('produtos.index', compact('data', 'categorias', 'stats'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Se não enviou arquivo, busca imagem automaticamente pelo nome do produto na Unsplash
            if (empty($file_name)) {
                $imgFromUnsplash = $this->produtoImagemService->downloadFromUnsplash($request->nome);
                if ($imgFromUnsplash) {
                    $file_name = $imgFromUnsplash;
                }
            }
            ...
            $request->merge([
                ...
                'perc_pis' => $request->perc_pis ? __convert_value_bd($request->perc_pis) : 0,
                'perc_cofins' => $request->perc_cofins ? __convert_value_bd($request->perc_cofins) : 0,
                'perc_ipi' => $request->perc_ipi ? __convert_value_bd($request->perc_ipi) : 0,
                'perc_ibs' => $request->perc_ibs ? __convert_value_bd($request->perc_ibs) : 0,
                'perc_cbs' => $request->perc_cbs ? __convert_value_bd($request->perc_cbs) : 0,
...
public function update(Request $request, $id)
{
    ...
    try {
        ...
        // Se não enviou arquivo e não tem imagem, busca automaticamente pelo nome do produto na Unsplash
        if (empty($file_name)) {
            $imgFromUnsplash = $this->produtoImagemService->downloadFromUnsplash($request->nome ?? $item->nome);
            if ($imgFromUnsplash) {
                $file_name = $imgFromUnsplash;
            }
        }
...
public function removeImagem($id){
    ...
    return redirect()->back();
        ...
        $fileName = $this->produtoImagemService->downloadFromUnsplash($item->nome);
...
public function reajuste(Request $request){
    ...
    $stats = [
        'total' => Produto::where('empresa_id', request()->empresa_id)->count(),
        'filtrados' => count($data),
        'categorias' => count($categorias),
        'marcas' => count($marcas)
    ];

    return view('produtos.reajuste', compact('data', 'categorias', 'marcas', 'stats'));
}

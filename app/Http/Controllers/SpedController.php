...
class SpedController extends Controller
{
    ...
    public function index(){
        ...
        // Estatísticas para exibir na view
        $stats = null;
        $spedFileInfo = null;

        $empresaId = request()->empresa_id;
        if (!$empresaId && auth()->check()) {
            // Tenta pegar a primeira empresa do usuário logado
            $primeiraEmpresa = Empresa::where('status', 1)->first();
            if ($primeiraEmpresa) {
        ...
        if ($empresaId) {
            $stats = [
                'nfe_saida' => Nfe::where('empresa_id', $empresaId)
                    ->where('tpNF', 1)
                    ->where('estado', 'aprovado')
                    ->whereDate('data_emissao_saida', '>=', $firstDate)
                    ->whereDate('data_emissao_saida', '<=', $lastDate)
                    ->count(),
                'nfce' => Nfce::where('empresa_id', $empresaId)
                    ...
                    ->count(),
                'nfe_entrada' => Nfe::where('empresa_id', $empresaId)
                    ->where('tpNF', 0)
                    ->where('estado', 'aprovado')
                    ->whereDate('created_at', '>=', $firstDate)
                    ->whereDate('created_at', '<=', $lastDate)
                    ->count(),
                'escritorio_configurado' => EscritorioContabil::where('empresa_id', $empresaId)->exists(),
                'sped_configurado' => SpedConfig::where('empresa_id', $empresaId)->exists(),
            ];

            // Verifica se já existe arquivo SPED gerado
            $cnpj = Empresa::where('id', $empresaId)->value('cpf_cnpj');
            if ($cnpj) {
                ...
                if (file_exists($filePath)) {
                    $spedFileInfo = [
                        ...
                        'url' => asset("sped_files/SPED-EFD-$cnpj.txt"),
                        'linhas' => count(file($filePath)),
                    ];
                }
            }
        }

        return view('sped.index', compact(
            'firstDate', 'lastDate', 'stats', 'spedFileInfo'
        ));
    }
    ...
    public function store(Request $request){
        ...
        $empresa = Empresa::with('cidade')->findOrFail($this->empresa_id);
        $config = $empresa;
        ...
        $std->FONE = preg_replace('/[^0-9]/', '', $config->celular);
        ...
        $vendasPdv = Nfce::whereDate('created_at', '>=', $dataInicial)
        ->whereDate('created_at', '<=', $dataFinal)
        ->where('estado', 'aprovado')
        ->where('empresa_id', $this->empresa_id)
        ->get();

        $compras = Nfe::whereDate('created_at', '>=', $dataInicial)
        ->whereDate('created_at', '<=', $dataFinal)
        ->where('estado', 'aprovado')
        ->where('tpNF', 0)
        ->where('empresa_id', $this->empresa_id)
        ...
        $comprasImportadas = Nfe::whereDate('created_at', '>=', $dataInicial)
        ->whereDate('created_at', '<=', $dataFinal)
        ->where('estado', 'aprovado')
        ->where('tpNF', 0)
        ->where('chave_importada', 1)
        ->where('empresa_id', $this->empresa_id)
        ->get();

        $comprasManifesto = Nfe::whereDate('created_at', '>=', $dataInicial)
        ->whereDate('created_at', '<=', $dataFinal)
        ->where('estado', 'aprovado')
        ->where('tpNF', 0)
        ->where('chave', '!=', '')
        ...
        foreach($dataXml as $l){
            ...
            foreach($itens as $item){
                ...
                $p = Produto::where(function($q) use ($cProd) {
                    $q->where('id', $cProd)
                      ->orWhere('referencia', $cProd);
                ...
                if($l['xml_importado'] == 1 || ($estoqueProduto != null && $estoqueProduto->quantidade > 0) && $inventario == 1){
                    ...
                    $firstIcms = $arr[0] ?? new \stdClass();
                    $cst_csosn = $firstIcms->CST ?? $firstIcms->CSOSN ?? '00';
                    $pICMS = $arr[0]->pICMS ?? 0;
        ...
        foreach($dataXml as $key => $l){
            ...
            foreach($itens as $item){
                ...
                $arr = (array_values((array)$imposto->ICMS));
                $firstIcms = $arr[0] ?? new \stdClass();
                $vBC = (float)($firstIcms->vBC ?? 0);
                $vBCST = (float)($firstIcms->vBCST ?? 0);
                $vICMSST = (float)($firstIcms->vICMSST ?? 0);
                $vICMS = (float)($firstIcms->vICMS ?? 0);
                $pRedBC = (float)($firstIcms->pRedBC ?? 0);
                $cst_csosn = $firstIcms->CST ?? $firstIcms->CSOSN ?? '00';
                $cst_csosn = (string)$cst_csosn;
                if(strlen($cst_csosn) == 2){
                    $cst_csosn = "0".$cst_csosn;
                ...
                $ipiObj = $imposto->IPI ?? null;
                $vIPI = 0;
                if ($ipiObj) {
                    $arr = (array_values((array)$ipiObj));
                    if(isset($arr[1])){
    ...
    private function getCompra($chave){
        $item = Nfe::where('chave', $chave)->where('empresa_id', $this->empresa_id)
        ->first();
        return $item;
    }

...
class UsuarioController extends Controller
{
    ...
    public function __construct(UploadUtil $util)
    {
        ...
        // Bypass para superadmin: o middleware Spatie não conhece __isMaster(),
        // por isso verificamos antes de delegar a validação de permissão.
        $this->middleware(function ($request, $next) {
            if (__isMaster()) return $next($request);
            return app(\Spatie\Permission\Middlewares\PermissionMiddleware::class)
                ->handle($request, $next, 'usuarios_create');
        })->only(['create', 'store']);

        $this->middleware(function ($request, $next) {
            if (__isMaster()) return $next($request);
            return app(\Spatie\Permission\Middlewares\PermissionMiddleware::class)
                ->handle($request, $next, 'usuarios_edit');
        })->only(['edit', 'update']);

        $this->middleware(function ($request, $next) {
            if (__isMaster()) return $next($request);
            return app(\Spatie\Permission\Middlewares\PermissionMiddleware::class)
                ->handle($request, $next, 'usuarios_view');
        })->only(['show', 'index']);

        $this->middleware(function ($request, $next) {
            if (__isMaster()) return $next($request);
            return app(\Spatie\Permission\Middlewares\PermissionMiddleware::class)
                ->handle($request, $next, 'usuarios_delete');
        })->only(['destroy']);
    }
    ...
    public function create(Request $request)
    {
        ...
        $formType = 'create';
        return view('usuarios.create', compact('roles', 'formType'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $formType = 'edit';
        return view('usuarios.edit', compact('item', 'roles', 'formType'));
    }

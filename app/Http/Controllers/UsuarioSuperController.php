...
class UsuarioSuperController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            session()->flash("flash_error", "Não é possível excluir o usuário: ele possui vínculos no sistema (ex: vendas, caixas).");
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->empresa) {
                $usuarioEmpresa = UsuarioEmpresa::where('usuario_id', $id)->first();
                if ($usuarioEmpresa) {
                    $usuarioEmpresa->empresa_id = $request->empresa;
                    $usuarioEmpresa->save();
                } else {

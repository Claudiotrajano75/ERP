...
class Empresa extends Model
{
    ...
    protected $fillable = [
        ...
        'perc_ap_cred', 'mensagem_aproveitamento_credito', 'simples_hibrido'
    ];
    ...
    /**
     * Accessor para senha: descriptografa automaticamente quando lida.
     * Se senha_encrypted estiver preenchido, retorna o valor descriptografado.
     * Se não (dado legado antes da migration), retorna o texto plano.
     */
    public function getSenhaAttribute($value)
    {
        if (!empty($this->senha_encrypted)) {
            try {
                return \Illuminate\Support\Facades\Crypt::decryptString($this->senha_encrypted);
            } catch (\Exception $e) {
                // Se falhar na descriptografia (ex: APP_KEY trocada), fallback para valor legado
                return $value;
            }
        }
        return $value;
    ...
    /**
     * Mutator para senha: criptografa automaticamente quando gravada.
     */
    public function setSenhaAttribute($value)
    {
        $this->attributes['senha'] = $value;
        if (!empty($value)) {
            $this->attributes['senha_encrypted'] = \Illuminate\Support\Facades\Crypt::encryptString($value);
        } else {
            $this->attributes['senha_encrypted'] = null;
        }
    }

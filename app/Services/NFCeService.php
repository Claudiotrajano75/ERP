...
class NFCeService
{
	...
	public function __construct($config, $empresa)
	{
		...
		$soapCurl->setSecurityLevel(true);
		$soapCurl->timeout($this->timeout);
	...
	public function gerarXml($item)
	{
		...
		foreach ($item->itens as $itemCont => $i) {
            ...
            if (!$isExemptIBSCBS) {
                ...
                // IBS (Dividido entre UF e Município - Transição 2026)
                $stdIBSCBS->gIBSUF_pIBSUF = $this->format($percIbs, 4);
                $stdIBSCBS->gIBSUF_vIBSUF = $this->format($stdProd->vProd * ($percIbs / 100));

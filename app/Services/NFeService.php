...
class NFeService{
	...
	public function gerarXml($item){
		$this->nfe = new Make($this->config['schemes'] ?? null);
		$nfe = $this->nfe;
		...
		foreach ($item->itens as $itemCont => $i) {
            ...
            $crt = $stdEmit->CRT;
            // CRT 1 (Simples) e 4 (MEI) são dispensados de informar IBS/CBS em 2026.
            // CRT 2 e 3 (Regime Normal/Sublimite) devem informar as alíquotas teste.
            $isExemptIBSCBS = ($crt == 1 || $crt == 4);

            if (!$isExemptIBSCBS) {
                $stdIBSCBS = new \stdClass();
                $stdIBSCBS->item = $itemCont;
                $stdIBSCBS->CST = '000'; // Tributação integral
                $stdIBSCBS->cClassTrib = '000001'; 
                $stdIBSCBS->vBC = $stdProd->vProd;

                if($item->natureza->perc_ibs){
                    $i->perc_ibs = $item->natureza->perc_ibs;
                }
                if($item->natureza->perc_cbs){
                    $i->perc_cbs = $item->natureza->perc_cbs;
                }

                // Alíquotas de teste 2026: padrão 0.1% IBS / 0.9% CBS
                $percIbs = $i->perc_ibs ?? 0.1; 
                $percCbs = $i->perc_cbs ?? 0.9;

                $stdIBSCBS->gIBSUF_pIBSUF = $this->format($percIbs, 4);
                $stdIBSCBS->gIBSUF_vIBSUF = $this->format($stdIBSCBS->vBC * ($percIbs / 100));
                
                $stdIBSCBS->gIBSMun_pIBSMun = $this->format(0, 4);
                $stdIBSCBS->gIBSMun_vIBSMun = $this->format(0, 2);
                
                $stdIBSCBS->vIBS = $this->format($stdIBSCBS->gIBSUF_vIBSUF + $stdIBSCBS->gIBSMun_vIBSMun);

                // CBS
                $stdIBSCBS->gCBS_pCBS = $this->format($percCbs, 4);
                $stdIBSCBS->gCBS_vCBS = $this->format($stdProd->vProd * ($percCbs / 100));
                $stdIBSCBS->vCBS = $stdIBSCBS->gCBS_vCBS;
        ...
        // Totalizador IBS/CBS (Grupo W03) - Obrigatório NT 2025.002
	...
	public function transmitir($signXml, $chave){
		try{
			...
			try {
				...
				$resNumero = $std->cStat == 103 ? $std->infRec->nRec : ($std->protNFe->infProt->nProt ?? $std->cStat);
				...
				return [
					'erro' => 1,
					'error' => "Erro ao autorizar: " . $e->getMessage(),
					'protocolo' => $st->toArray($protocolo),
					'recibo' => isset($recibo) ? $recibo : null
				];
			}

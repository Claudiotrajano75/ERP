// ============================================================
// TOAST NOTIFICATION SYSTEM (Compact Layout)
// ============================================================
...
$(function(){
	...
	$(document).on('click', '.categorias-pagination a', function(event){
		...
		$.get(path_url + 'api/frenteCaixa/categorias-page?page='+page, { empresa_id: $('#empresa_id').val() })
		.done((res) => {
...
function removeItem(code){
	var nome = $('.product-line-'+code).find('.pdv-cart-title').text() || 'Item';
	swal({
		...
		if (isConfirm) {
			...
			calculaTotal()
			pdvToast(nome + ' removido do carrinho', 'info', 'Removido');
		} else {
...
$(document).on('click', '.increment-decrement', function(e){
	...
	setTimeout(() => {
		...
		calculaTotal()
		if(bt == '+'){
			checkLowStock($('.product-line-'+code))
		}
	}, 10)
});
...
function buscarPorReferencia(barcode) {
	...
	.done((e) => {
		...
		calcTotal();
		setTimeout(function() {
		    $('.total').addClass('pdv-total-bounce');
		    setTimeout(function() {
		        $('.total').removeClass('pdv-total-bounce');
		    }, 500);
		}, 20);
	})
	.fail((e) => {
...
function addProduto(id){
	...
	if (abertura) {
		$.post(path_url + 'api/frenteCaixa/add-produto', { produto_id: id, lista_id: $('#lista_id').val(), qtd: qtd })
		.done((res) => {
			let idDup = 0
			if(agrupar_itens == 1){
			...
			setTimeout(() => {
				if(idDup == 0){
					var $newItem = $(res).addClass('pdv-item-new');
					$('.itens-cart').append($newItem);
					pdvAtualizarContagemCarrinho(true);
					setTimeout(function() {
					    $('.total').addClass('pdv-total-bounce');
					    setTimeout(function() {
			...
			calculaTotal()
			beepSucesso()
			pdvToast('Produto adicionado ao carrinho', 'success', 'Produto')
			$('#inp-pesquisa').focus()
		}).fail((err) => {
			console.log(err)
...
function pdvAtualizarContagemCarrinho(animar) {
    ...
    var count = $('.itens-cart .products').length || 0;
...
// ============================================================
// ATALHOS DE TECLADO - PDV (Layout Compact)
// ============================================================
$(function() {
	$(document).on('keydown', function(e) {
		var key = e.key;
		// Previne comportamento padrão do navegador para F1-F8
		if (['F1','F2','F3','F4','F5','F8'].indexOf(key) >= 0) {
			e.preventDefault();
		}
		...
		switch(key) {
			...
			case 'F3':
				...
				break;
			case 'F4':
				$('#btn-finalizar').trigger('click');
				break;
			case 'F5':
				if ($('#modal_finalizar_pdv2').hasClass('show')) {
...
$('.btn-vendas-suspensas').click(() => {
	...
	.fail((e) => {
		console.log(e);
		pdvToast('Erro ao carregar vendas suspensas', 'error', 'Erro');
	});
})
...
// ============================================================
// CORES POR TIPO DE PAGAMENTO (Layout Compact)
// ============================================================
...
$(function() {
    // Aplica cor nos selects de pagamento do modal de finalização
...
// Aplica cores de pagamento em novas linhas adicionadas dinamicamente
...
function checkLowStock($row) {
    ...
    let nomeProduto = $row.attr('data-nome-produto') || $row.find('.title').text();

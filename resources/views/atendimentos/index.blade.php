        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-header modulo-header-gradient py-3 px-4">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        <h4 class="mb-1 modulo-title d-flex align-items-center gap-2"><i class="ri-calendar-todo-line"></i> Dias de Atendimento</h4>
                        <p class="text-muted mb-0 modulo-subtitle fs-13">Configure os dias da semana que cada colaborador estará disponível.</p>
                    </div>
                    <div>
                        @can('atendimentos_create')
                        <a href="{{ route('atendimentos.create') }}" class="btn btn-light btn-sm px-3 text-dark"><i class="ri-add-circle-line align-middle me-1"></i> Cadastrar</a>
                        @endcan
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                ...
                <div class="modulo-glass-filter p-3 mb-4">
                    ...
                    <div class="row g-2 align-items-end">
                        <div class="col-md-8 col-12">
                            {!!Form::select('funcionario_id', 'Funcionário', ['' => 'Selecione'] + $funcionarios->pluck('nome', 'id')->all())->attrs(['class' => 'select2 form-select form-select-sm'])!!}
                        </div>
                        <div class="col-md-4 col-12 ms-auto">
                            <div class="d-flex gap-2">
                                ...
                                <a class="btn btn-danger btn-sm px-3" href="{{ route('atendimentos.index') }}"><i class="ri-eraser-line me-1"></i> Limpar</a>
                            </div>
                        </div>
                    </div>
                    {!!Form::close()!!}
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><span class="badge bg-light text-dark border p-2 fs-11">{{ $item->diaStr() }}</span></td>
                                    <td class="text-end">
                                        <form action="{{ route('atendimentos.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="m-0">
                                            @method('delete') @csrf
                                            <div class="modulo-action-group">
                                                @can('atendimentos_edit')
                                                <a class="btn btn-warning btn-sm text-white" href="{{ route('atendimentos.edit', [$item->id]) }}" title="Editar"><i class="ri-pencil-line"></i></a>
                                                @endcan
                                ...
                                @empty
                                <tr><td colspan="3"><div class="modulo-empty"><i class="ri-inbox-2-line"></i><p>Nenhum vínculo de atendimento cadastrado.</p></div></td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="modulo-footer">
                    <div>
                        <span class="modulo-total-label">Total de vínculos:</span>
                        <span class="modulo-total-value">{{ $data->count() }}</span>
                    </div>
                </div>

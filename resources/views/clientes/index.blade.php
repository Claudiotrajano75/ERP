        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-header modulo-header-gradient py-3 px-4">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        <h4 class="mb-1 modulo-title d-flex align-items-center gap-2"><i class="ri-user-follow-line"></i> Gestão de Clientes</h4>
                        <p class="text-muted mb-0 modulo-subtitle fs-13">Cadastre clientes, acompanhe históricos de vendas e saldos de cashback.</p>
                    </div>
                    <div class="d-flex gap-2">
                        @can('clientes_create')
                        <a href="{{ route('clientes.create') }}" class="btn btn-light btn-sm px-3 text-dark"><i class="ri-add-circle-line align-middle me-1"></i> Novo Cliente</a>
                        <a href="{{ route('clientes.import') }}" class="btn btn-light btn-sm px-3 text-dark"><i class="ri-file-upload-line align-middle me-1"></i> Importar</a>
                        @endcan
                    </div>
                </div>
            </div>
            ...
            <div class="card-body p-4">
                ...
                <div class="modulo-glass-filter p-3 mb-4">
                    ...
                    <div class="row g-2 align-items-end">
                        <div class="col-md-3 col-12">
                            {!!Form::text('razao_social', 'Nome / Razão Social')->attrs(['class' => 'form-control form-control-sm'])!!}
                        </div>
                        <div class="col-md-3 col-12">
                            {!!Form::text('cpf_cnpj', 'CPF/CNPJ')->attrs(['class' => 'cpf_cnpj form-control form-control-sm', 'type' => 'tel'])!!}
                        </div>
                        ...
                        <div class="col-md-2 col-12 ms-auto">
                            <div class="d-flex gap-2">
                                ...
                                <a class="btn btn-danger btn-sm px-3" href="{{ route('clientes.index') }}"><i class="ri-eraser-line me-1"></i> Limpar</a>
                            </div>
                        </div>
                    </div>
                    {!!Form::close()!!}
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($item->status)
                                        <span class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1 fs-11"><i class="ri-check-line me-1"></i>Ativo</span>
                                        @else
                                        <span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2 py-1 fs-11"><i class="ri-close-line me-1"></i>Inativo</span>
                                        @endif
                                    </td>
                                    <td class="text-muted fs-12">{{ __data_pt($item->created_at) }}</td>
                                    <td class="text-end">
                                        <form action="{{ route('clientes.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="m-0">
                                            @method('delete') @csrf
                                            <div class="modulo-action-group">
                                                @can('clientes_edit')
                                                <a class="btn btn-warning btn-sm text-white" href="{{ route('clientes.edit', [$item->id]) }}" title="Editar"><i class="ri-pencil-line"></i></a>
                                                @endcan
                                                <a title="Cashback" class="btn btn-dark btn-sm text-white" href="{{ route('clientes.cash-back', [$item->id]) }}"><i class="ri-coins-line"></i></a>
                                                <a title="Histórico" class="btn btn-info btn-sm text-white" href="{{ route('clientes.historico', [$item->id]) }}"><i class="ri-file-list-3-line"></i></a>
                                                @can('clientes_delete')
                                ...
                                @empty
                                <tr><td colspan="{{ (Auth::user()->can('clientes_delete') ? 1 : 0) + 8 }}"><div class="modulo-empty"><i class="ri-inbox-2-line"></i><p>Nenhum cliente cadastrado.</p></div></td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="modulo-footer">
                    <div>
                        @can('clientes_delete')
                        <form action="{{ route('clientes.destroy-select') }}" method="post" id="form-delete-select" class="m-0">
                            @method('delete') @csrf

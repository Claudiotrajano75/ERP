            ...
            <div class="card-header modulo-header-gradient py-3 px-4">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        ...
                        <p class="text-muted mb-0 modulo-subtitle fs-13">Gerencie as compras efetuadas, dê entrada nos estoques importando arquivos XML de fornecedores ou manualmente.</p>
                    </div>
            ...
            <div class="card-body p-4">
                ...
                <div class="modulo-glass-filter p-3 mb-4">
                    ...
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-md-2 col-6">
                            {!!Form::select('estado', 'Status / Estado', [
                                'novo' => 'Novas',
                        ...
                        <div class="col-md-3 col-6">
                            {!!Form::select('local_id', 'Local / Filial', ['' => 'Selecione'] + __getLocaisAtivoUsuario()->pluck('descricao', 'id')->all())
                            ->attrs(['class' => 'select2 form-select'])!!}
                        </div>
                        @endif
                        <div class="col-md-3 col-12 ms-auto">
                            <div class="d-flex gap-2">
                                ...
                                <a class="btn btn-danger btn-sm px-3" href="{{ route('compras.index') }}">
                ...
                @if($contigencia != null)
                <div class="alert alert-danger border-danger-subtle bg-danger-subtle text-danger p-3 mb-4 d-flex align-items-center">
                    <i class="ri-error-warning-line me-2 fs-20"></i>
                    <div>
                        <strong class="d-block">Contingência Ativada!</strong>
                        <span>Tipo: {{ $contigencia->tipo }} | Início em: {{ __data_pt($contigencia->created_at) }}</span>
                    </div>
                </div>
                @endif
                                ...
                                <td class="text-end">
                                    <form action="{{ route('nfe.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="m-0">
                                        ...
                                        <div class="modulo-action-group">
                                            ...
                                            <button type="button" class="btn btn-dark btn-sm text-white" onclick="info('{{$item->motivo_rejeicao}}', '{{$item->chave}}', '{{$item->estado}}', '{{$item->recibo}}')" title="Ver Detalhes do Retorno">
                                            ...
                                            @can('compras_edit')
                                            <a class="btn btn-warning btn-sm text-white" href="{{ route('nfe.edit', $item->id) }}" title="Editar Compra">
                                            ...
                                            @can('compras_delete')
                                            <button type="button" class="btn btn-danger btn-sm btn-delete" title="Excluir Pré-compra"><i class="ri-delete-bin-line"></i></button>
                                            @endcan
                                            @can('nfe_transmitir')
                                            ...
                                            <a href="{{ route('compras.info-validade', $item->id) }}" title="Preencher Vencimentos/Lotes" type="button" class="btn btn-info btn-sm text-white"><i class="ri-pencil-line"></i></a>
                                            @endif

                                            <a class="btn btn-info btn-sm text-white" title="Imprimir Pedido de Compra" target="_blank" href="{{ route('nfe.imprimirVenda', [$item->id]) }}">
                                                <i class="ri-printer-line"></i>
                                            </a>

                                            <a class="btn btn-light btn-sm" title="Gerar Etiquetas de Preço/Barras" target="_blank" href="{{ route('compras.etiqueta', [$item->id]) }}">
    ...
    <div class="modal-dialog">
        <div class="modal-content text-dark">
            <div class="modal-header">
                <h5 class="modal-title" id="modalCancelarLabel">Cancelar Nota Fiscal Eletrônica <strong class="ref-numero text-danger"></strong></h5>
...
<div class="modal fade" id="modal-corrigir" tabindex="-1" aria-labelledby="modalCorrigirLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content text-dark">
            <div class="modal-header">
                <h5 class="modal-title" id="modalCorrigirLabel">Emitir Carta de Correção (CC-e) <strong class="ref-numero text-warning"></strong></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row g-2">

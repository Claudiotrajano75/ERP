...
.modulo-header-gradient .modulo-title i {
    background: rgba(255,255,255,0.12); padding: 8px; border-radius: 10px; color: #a8b5ff;
}
...
.modulo-table-wrap thead th {
    background: #f8f9fc; color: #5a5a7a; font-weight: 700; font-size: 11px; text-transform: uppercase;
    letter-spacing: 0.4px; padding: 12px 14px; border-bottom: 2px solid #e8eaf6;
}
...
.modulo-footer .modulo-total-value { font-size: 18px; font-weight: 800; letter-spacing: -0.3px; }
                    ...
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-md-2 col-12 ms-auto">
                            <div class="d-flex gap-2">
                                ...
                                <a class="btn btn-outline-secondary btn-sm px-3" href="{{ route('conta-receber.index') }}">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @endcan
                                    <td>
                                        <span class="fw-semibold text-dark">{{ $item->cliente ? $item->cliente->razao_social : '--' }}</span>
                                        @if($item->cliente)
                                        <span class="text-muted d-block fs-11">{{ $item->cliente->cpf_cnpj }}</span>
                                        @endif
                                    </td>
                                    @if(__countLocalAtivo() > 1)
                                    <td><span class="badge bg-danger-subtle text-danger border border-danger-subtle fs-11">{{ $item->localizacao->descricao }}</span></td>
                                    @endif
                                    ...
                                    <td>
                                        ...
                                        @if(!$item->status)
                                        <span class="badge bg-danger-subtle text-danger fs-10 ms-1">{{ $item->diasAtraso() }}</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($item->status)
                                    ...
                                    <td>
                                        @if($item->nfce)
                                        <a href="{{ route('nfce.show', [$item->nfce->id]) }}" class="badge bg-primary-subtle text-primary border border-primary-subtle py-1 px-1.5">PDV #{{ $item->nfce->numero_sequencial }}</a>
                                        @elseif($item->nfe)
                                        <a href="{{ route('nfe.show', [$item->nfe->id]) }}" class="badge bg-dark-subtle text-dark border border-dark-subtle py-1 px-1.5">Pedido #{{ $item->nfe->numero_sequencial }}</a>
                                        @else
                                        ...
                                        <form action="{{ route('conta-receber.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="m-0">
                                            ...
                                            <div class="modulo-action-group">
                                                @if(!$item->status)
                                                    ...
                                                    <a class="btn btn-warning btn-sm text-white" href="{{ route('conta-receber.edit', [$item->id]) }}" title="Editar">
                                                    ...
                                                    @endcan
                                                    @can('conta_receber_edit')
                                                    <a href="{{ route('conta-receber.pay', $item) }}" class="btn btn-success btn-sm text-white" title="Receber">
                                                ...
                                                @if(!$item->boleto && !$item->status)
                                                    @can('boleto_create')
                                                    <a class="btn btn-secondary btn-sm text-white" href="{{ route('boleto.create', [$item->id]) }}" title="Gerar Boleto">
                                                ...
                                                @else
                                                    @can('boleto_view')
                                                        @if($item->boleto)
                                                        <a class="btn btn-info btn-sm text-white" href="{{ route('boleto.show', [$item->id]) }}" title="Ver Boleto">

        ...
        <div class="card border-0 shadow-sm text-dark modulo-form-card">
            ...
            <div class="card-header modulo-header-gradient py-3 px-4">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        <h4 class="mb-1 modulo-title d-flex align-items-center gap-2"><i class="ri-bank-line"></i> Contas da Empresa</h4>
                    ...
                    <div>
                        ...
                        <a href="{{ route('contas-empresa.create') }}" class="btn btn-light btn-sm px-3 text-dark"><i class="ri-add-circle-line align-middle me-1"></i> Nova Conta</a>
                        @endcan
                    </div>
                </div>
            </div>
                            ...
                            <div class="card-body">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h4 class="text-uppercase fs-12 mt-0 text-white-50">Saldo Consolidado</h4>
                                        <h3 class="my-2 text-white fs-18">R$ {{ __moeda($data->sum('saldo')) }}</h3>
                                    </div>
                    ...
                    <div class="col-md-6 col-6">
                        <div class="card widget-icon-box text-bg-success mb-0">
                            <div class="card-body">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h4 class="text-uppercase fs-12 mt-0 text-white-50">Contas Ativas</h4>
                                        <h3 class="my-2 text-white fs-18">{{ $data->where('status', 1)->count() }}</h3>
                                    </div>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title bg-white bg-opacity-25 text-white rounded rounded-3 fs-3 widget-icon-box-avatar shadow">
                ...
                <div class="modulo-glass-filter p-3 mb-4">
                    ...
                    <div class="row g-2 align-items-end">
                        <div class="col-md-4 col-12">
                            {!!Form::select('local_id', 'Local', ['' => 'Todos os Locais'] + __getLocaisAtivoUsuario()->pluck('descricao', 'id')->all())->attrs(['class' => 'form-select form-select-sm'])!!}
                        </div>
                        <div class="col-md-3 col-12 ms-auto">
                            <div class="d-flex gap-2">
                                ...
                                <a class="btn btn-outline-secondary btn-sm px-3" href="{{ route('contas-empresa.index') }}"><i class="ri-eraser-line me-1"></i> Limpar</a>
                            </div>
                        </div>
                    </div>
                    {!!Form::close()!!}
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="fw-medium text-dark">{{ $item->nome }}</td>
                                    <td class="text-muted fs-12">{{ $item->plano->descricao }}</td>
                                    <td>{{ $item->banco ?: '--' }}</td>
                                    <td>{{ $item->agencia ?: '--' }}</td>
                                    <td>{{ $item->conta ?: '--' }}</td>
                                    <td>
                                    ...
                                    <td class="fw-bold @if($item->saldo >= 0) text-success @else text-danger @endif">R$ {{ __moeda($item->saldo) }}</td>
                                    <td class="text-end">
                                        <form action="{{ route('contas-empresa.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="m-0">
                                            @csrf @method('delete')
                                            <div class="modulo-action-group">
                                                @can('contas_empresa_edit')
                                                <a class="btn btn-warning btn-sm text-white" href="{{ route('contas-empresa.edit', [$item->id]) }}" title="Editar"><i class="ri-pencil-line"></i></a>
                                                @endcan
                                                ...
                                                @endcan
                                                <a href="{{ route('contas-empresa.show', $item) }}" class="btn btn-info btn-sm text-white" title="Extrato"><i class="ri-file-list-3-line"></i></a>
                                            </div>
                                        </form>
                                    </td>
                                </tr>
                                @empty
                                <tr><td colspan="{{ __countLocalAtivo() > 1 ? 9 : 8 }}"><div class="modulo-empty"><i class="ri-inbox-2-line"></i><p>Nenhuma conta bancária configurada.</p></div></td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>

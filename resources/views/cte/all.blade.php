@extends('layouts.app', ['title' => 'CT-e — Todas as Empresas'])
        ...
        <div class="card">
            ...
            <div class="card-body">
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h4 class="text-uppercase fs-12 mt-0 text-white-50">Total de CT-e</h4>
                                        <h3 class="my-1 text-white fs-20 fw-bold">{{ number_format($stats['total'] ?? 0, 0, ',', '.') }}</h3>
                                        <p class="mb-0 text-white-50 fs-11">Documentos emitidos</p>
                                    </div>
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        ...
                                        <p class="mb-0 text-white-50 fs-11">CT-e aprovados</p>
                                    </div>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title bg-white bg-opacity-25 text-white rounded rounded-3 fs-3 widget-icon-box-avatar shadow">
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        ...
                                        <p class="mb-0 text-white-50 fs-11">CT-e cancelados</p>
                                    </div>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title bg-white bg-opacity-25 text-white rounded rounded-3 fs-3 widget-icon-box-avatar shadow">
                    ...
                    <div class="col-md-3 col-6">
                        <div class="card widget-icon-box text-bg-primary mb-0 shadow-sm border-0">
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h4 class="text-uppercase fs-12 mt-0 text-white-50">Total Transporte</h4>
                ...
                <div class="col-lg-12 mb-3">
                    ...
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-md-3 col-12 d-flex gap-2">
                            ...
                            <a id="clear-filter" class="btn btn-danger px-3" href="{{ route('cte-all') }}">
                        ...
                        <table class="table table-centered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-medium text-dark">{{ $item->remetente ? $item->remetente->razao_social : '--' }}</span>
                                    </td>
                                    <td>
                                        <span class="fw-medium text-dark">{{ $item->destinatario ? $item->destinatario->razao_social : '--' }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="fw-bold text-success">
                                            R$ {{ number_format($item->valor_transporte, 2, ',', '.') }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <form action="{{ route('cte.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="m-0">
                                            @method('delete')
                                            @csrf
                                            <div class="modulo-action-group">
                                                ...
                                                <button title="Cancelar CT-e" type="button" class="btn btn-danger btn-sm text-white" onclick="cancelar('{{$item->id}}', '{{$item->numero}}')">
                                                    <i class="ri-close-circle-line"></i>
                                                </button>
                                                @endif

                                                @if($item->estado == 'aprovado' || $item->estado == 'rejeitado')
                                                <button type="button" class="btn btn-dark btn-sm text-white" title="Detalhes do Retorno SEFAZ" onclick="info('{{$item->motivo_rejeicao}}', '{{$item->chave}}', '{{$item->estado}}', '{{$item->recibo}}')">
                                                    <i class="ri-information-line"></i>
                                                </button>
                                                @endif

                                                @if($item->estado == 'novo' || $item->estado == 'rejeitado')
                                                <a target="_blank" title="Visualizar XML Temporário" class="btn btn-light btn-sm text-dark" href="{{ route('cte.xml-temp', $item->id) }}">
                                                    <i class="ri-file-code-line"></i>
                                                </a>
                                                <button title="Transmitir CT-e ao SEFAZ" type="button" class="btn btn-success btn-sm text-white" onclick="transmitir('{{$item->id}}')">
                                ...
                                <tr>
                                    <td colspan="10" class="text-center text-muted py-4">
                                        ...
                                        Nenhum Conhecimento de Transporte (CT-e) encontrado para os filtros selecionados.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-3">
                    <div>
                        <span class="text-muted fs-12">Exibindo {{ $data->count() }} de {{ $data->total() }} conhecimentos de transporte</span>
                    </div>
                    <div>
    ...
    <div class="modal-dialog">
        <div class="modal-content text-dark">
            <div class="modal-header">
                <h5 class="modal-title" id="modalCancelarLabel">Cancelar CT-e <strong class="ref-numero text-danger"></strong></h5>
...
<script type="text/javascript">
    function info(motivo_rejeicao, chave, estado, recibo) {
        if (estado == 'rejeitado') {
            ...
            swal("CT-e Rejeitado", text, "warning");
        } else {
            let text = "Chave: " + chave + "\nRecibo: " + recibo + "\n";
            swal("CT-e Autorizado", text, "success");
        }
    }
</script>

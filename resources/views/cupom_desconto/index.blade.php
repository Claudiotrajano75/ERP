                    ...
                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                        <div>
                            ...
                            <p class="mb-0 modulo-subtitle fs-13">
                                Gerencie cupons e descontos promocionais para seus clientes.
                            </p>
                        </div>
                        <a href="{{ route('cupom-desconto.create') }}" class="btn btn-light text-dark fw-semibold px-4 py-2">
                            <i class="ri-add-circle-fill me-1"></i> Novo Cupom
                        </a>
                    </div>
                </div>
                ...
                <div class="card-body bg-white p-4">
                    ...
                    <div class="filter-box">
                        ...
                        <div class="row align-items-end g-3">
                            ...
                            <div class="col-md-3">
                                {!!Form::date('data_expiracao', 'Data de expiração')->attrs(['class' => 'form-control'])!!}
                            </div>
                            <div class="col-md-3">
                                {!!Form::select('status', 'Status', ['' => 'Todos', '1' => 'Ativo', '0' => 'Desativados'])->attrs(['class' => 'form-select'])!!}
                            </div>
                            <div class="col-md-3">
                                ...
                                <a id="clear-filter" class="btn btn-light border px-3" href="{{ route('cupom-desconto.index') }}">
                    ...
                    <div class="table-responsive-sm">
                        <table class="table table-custom mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="fw-bold text-primary"><i class="ri-coupon-line me-1"></i> {{ $item->codigo }}</td>
                                    <td>
                                        <span class="fw-bold text-success">{{ __moeda($item->valor) }}</span>
                                        <small class="text-muted d-block">{{ strtoupper($item->tipo_desconto) }}</small>
                                    </td>
                                    <td>{{ $item->cliente ? $item->cliente->razao_social : 'Todos' }}</td>
                                    <td>R$ {{ __moeda($item->valor_minimo_pedido)}}</td>
                                    <td>
                                        <span class="badge bg-light text-dark border px-2 py-1 fs-13"><i class="ri-calendar-event-line me-1 text-muted"></i>{{ __data_pt($item->expiracao, 0) }}</span>
                                    </td>
                                    <td>
                                        @if($item->status)
                                    ...
                                    <td>
                                        <form action="{{ route('cupom-desconto.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="d-flex gap-1 flex-wrap">
                                            @method('delete')
                                            <a class="btn btn-warning btn-sm text-white" href="{{ route('cupom-desconto.edit', [$item->id]) }}" data-bs-toggle="tooltip" title="Editar">
                                ...
                                <tr>
                                    <td colspan="7" class="text-center py-5">
                                        <i class="ri-coupon-3-line fs-1 text-muted opacity-50 mb-3 d-block"></i>

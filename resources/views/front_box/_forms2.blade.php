...
<div class="row">
		...
		<div class="d-flex align-items-center">
			<div class="w-100 text-gray-600 position-relative">
				...
				<div class="input-group flex-nowrap">
					...
					<input type="text" id="inp-pesquisa" class="form-control border border-gray-300 py-3 pr-3"
						placeholder="Pesquise produto por código ou nome">
				</div>
	...
	<div class="col-md-6 col-12">
		<div class="card">
			<div class="card-body">
				<div class="row">
					<div class="col-md-3">
						<button type="button" class="btn pdv-top-btn btn-outline-secondary w-100"
							data-bs-toggle="modal" data-bs-target="#lista_precos"><i class="ri-cash-line"></i> Lista de
							preços</button>
					</div>
					<div class="col-md-3">
						<button type="button" class="btn pdv-top-btn btn-outline-dark w-100 btn-vendas-suspensas"
							data-bs-toggle="modal" data-bs-target="#vendas_suspensas"><i
								class="ri-time-fill"></i> Vendas suspensas</button>
					</div>
					<div class="col-md-3">
						<a href="{{ route('frontbox.create') }}" type="button" class="btn pdv-top-btn btn-outline-danger w-100 ">
							<i class="ri-shut-down-line"></i> Reiniciar venda</a>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="card">
			...
			<div class="row">
				<div class="card-body">
					<hr>
					<div class="col-12">
						<span class="badge bg-success"><i class="ri-shopping-bag-3-line me-1"></i>Itens da Venda <span class="pdv-cart-count badge bg-light text-success rounded-pill ms-1">0</span></span>
					</div>
						...
						@foreach ($item->itens as $key => $p)
							...
							<div class="row mt-1 products product-line-{{$code}}">
								...
								<div class="col-md-5 text-left cart-data">
									...
									<i class="ri-pencil-fill text-primary"
										onclick="editItem('{{$code}}', '{{$p->produto->id}}')"></i>
								...
								<input type="hidden" class="quantidade" name="quantidade[]"
									value="{{ __moeda($p->quantidade) }}">
								<input type="hidden" class="valor_unitario" name="valor_unitario[]"
									value="{{ __moeda($p->valor_unitario) }}">
								<input type="hidden" class="subtotal_item" name="subtotal_item[]"
									value="{{ __moeda($p->sub_total) }}">
				...
				<div class="card-body">
					...
					<div class="row g-1">
						<div class="col-md-4 col-12">
							<label class="form-label fw-semibold"><i class="ri-indeterminate-circle-line text-primary me-1"></i>Desconto</label>
							<div class="input-group">
								<input id="inp-desconto" type="tel" class="form-control moeda"
						...
						<div class="col-md-4 col-12">
							<label class="form-label fw-semibold"><i class="ri-add-circle-line text-warning me-1"></i>Acréscimo</label>
							<div class="input-group">
								<input value="{{ isset($item) ? __moeda($item->acrescimo) : '' }}" id="inp-acrescimo"
									type="tel" class="form-control moeda">
						...
						<div class="col-12">
							<label class="fw-semibold"><i class="ri-user-smile-line me-1"></i>Cliente</label>
							<div class="input-group flex-nowrap">
								...
								@can('clientes_create')
									<button class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#modal_novo_cliente"
										type="button">
	...
	->options(isset($item) && $item->funcionario ? [$item->funcionario->id => $item->funcionario->nome] : [])
						...
						<div class="col-12 mt-2">
							<button type="button" id="btn-finalizar" class="pdv-btn-finalizar">
								<i class="ri-checkbox-circle-line"></i> Finalizar R$ <strong class="total">0,00</strong>
							</button>
						</div>
					</div>
				</div>
					...
					<div class="col-md-3 col-12 mt-3">
						...
						<div class="list-group">
							...
							<button type="button" data-bs-toggle="modal" data-bs-target="#suprimento_caixa"
								class="btn pdv-sidebar-btn btn-outline-info w-100 mt-1">
								<i class="ri-add-box-line"></i>
								Suprimento de Caixa
							</button>

							<button type="button" data-bs-toggle="modal" data-bs-target="#sangria_caixa"
								class="btn pdv-sidebar-btn btn-outline-danger w-100 mt-1">
								<i class="ri-checkbox-indeterminate-line"></i>
								Sangria de Caixa
							</button>
							@if($isVendaSuspensa == 0)
								<button type="button" id="btn-suspender" class="btn pdv-sidebar-btn btn-outline-warning w-100 mt-1">
									<i class="ri-timer-line"></i>
									Suspender Venda
								</button>
							@else
								<a href="{{ route('frontbox.create') }}" class="btn pdv-sidebar-btn btn-outline-warning w-100 mt-1">
									<i class="ri-refresh-line"></i>
									Nova Venda
								</a>
							@endif

							<a href="{{ route('frontbox.index')}}" class="btn pdv-sidebar-btn btn-outline-primary w-100 mt-1">
...
@section('js')
	...
	<script src="/js/frente_caixa2.js?v={{time()}}"></script>

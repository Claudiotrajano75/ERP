@extends('layouts.app', ['title' => 'MDF-e — Todas as Empresas'])
        ...
        <div class="card">
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        <h4 class="modulo-title text-white">
                            <i class="ri-truck-line"></i> MDF-e — Todas as Empresas
                        </h4>
                        <p class="modulo-subtitle">
                            Visão global e gerenciamento dos Manifestos Eletrônicos de Documentos Fiscais emitidos no sistema.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body">
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h4 class="text-uppercase fs-12 mt-0 text-white-50">Total de MDF-e</h4>
                                        <h3 class="my-1 text-white fs-20 fw-bold">{{ number_format($stats['total'] ?? 0, 0, ',', '.') }}</h3>
                                        <p class="mb-0 text-white-50 fs-11">Manifestos gerados</p>
                                    </div>
                    ...
                    <div class="col-md-3 col-6">
                        <div class="card widget-icon-box text-bg-success mb-0 shadow-sm border-0">
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h4 class="text-uppercase fs-12 mt-0 text-white-50">Autorizados</h4>
                                        <h3 class="my-1 text-white fs-20 fw-bold">{{ number_format($stats['aprovadas'] ?? 0, 0, ',', '.') }}</h3>
                                        <p class="mb-0 text-white-50 fs-11">MDF-e aprovados</p>
                                    </div>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title bg-white bg-opacity-25 text-white rounded rounded-3 fs-3 widget-icon-box-avatar shadow">
                    ...
                    <div class="col-md-3 col-6">
                        <div class="card widget-icon-box text-bg-danger mb-0 shadow-sm border-0">
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h4 class="text-uppercase fs-12 mt-0 text-white-50">Cancelados</h4>
                                        <h3 class="my-1 text-white fs-20 fw-bold">{{ number_format($stats['canceladas'] ?? 0, 0, ',', '.') }}</h3>
                                        <p class="mb-0 text-white-50 fs-11">MDF-e cancelados</p>
                                    </div>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title bg-white bg-opacity-25 text-white rounded rounded-3 fs-3 widget-icon-box-avatar shadow">
                    ...
                    <div class="col-md-3 col-6">
                        <div class="card widget-icon-box text-bg-primary mb-0 shadow-sm border-0">
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h4 class="text-uppercase fs-12 mt-0 text-white-50">Valor das Cargas</h4>
                                        <h3 class="my-1 text-white fs-20 fw-bold">R$ {{ number_format($stats['valor_total'] ?? 0, 2, ',', '.') }}</h3>
                                        <p class="mb-0 text-white-50 fs-11">Soma das cargas aprovadas</p>
                                    </div>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title bg-white bg-opacity-25 text-white rounded rounded-3 fs-3 widget-icon-box-avatar shadow">
                ...
                <div class="col-lg-12 mb-3">
                    ...
                    <div class="row g-2 align-items-end">
                        <div class="col-md-3 col-12">
                            <label class="form-label"><i class="ri-building-line me-1"></i> Empresa Emitente</label>
                        ...
                        <div class="col-md-2 col-6">
                            ...
                            {!!Form::select('estado', '', [
                                '' => 'Todos os Estados',
                                'aprovado' => 'Autorizados / Aprovados',
                                'cancelado' => 'Cancelados',
                                'rejeitado' => 'Rejeitados',
                                'novo' => 'Novos / Pendentes'
                            ])->attrs(['class' => 'select2 form-select'])!!}
                        </div>
                        <div class="col-md-3 col-12 d-flex gap-2">
                            ...
                            <a id="clear-filter" class="btn btn-danger px-3" href="{{ route('mdfe-all') }}">
                        ...
                        <table class="table table-centered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="font-monospace fs-12 text-muted">{{ $item->cnpj_contratante ?: '--' }}</span>
                                    </td>
                                    <td>
                                        <span class="badge bg-light-subtle text-dark font-monospace fs-12">
                                            #{{ $item->mdfe_numero > 0 ? $item->mdfe_numero : '--' }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <form action="{{ route('mdfe.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="m-0">
                                            @method('delete')
                                            @csrf
                                            <div class="modulo-action-group">
                                                @if($item->estado_emissao == 'cancelado')
                                                <a class="btn btn-danger btn-sm text-white" target="_blank" href="{{ route('mdfe.imprimir-cancela', [$item->id]) }}" title="Imprimir Cancelamento">
                                                    <i class="ri-printer-line"></i>
                                                </a>
                                                @endif

                                                @if($item->estado_emissao == 'aprovado')
                                                <a class="btn btn-primary btn-sm text-white" target="_blank" href="{{ route('mdfe.imprimir', [$item->id]) }}" title="Imprimir DAMDFE">
                                                    <i class="ri-printer-line"></i>
                                                </a>
                                                <button title="Cancelar MDF-e" type="button" class="btn btn-danger btn-sm text-white" onclick="cancelar('{{$item->id}}', '{{$item->mdfe_numero}}')">
                                                    <i class="ri-close-circle-line"></i>
                                                </button>
                                                @endif

                                                @if($item->estado_emissao == 'aprovado' || $item->estado_emissao == 'rejeitado')
                                                <button type="button" class="btn btn-dark btn-sm text-white" title="Detalhes do Retorno SEFAZ" onclick="info('{{$item->motivo_rejeicao}}', '{{$item->chave}}', '{{$item->estado_emissao}}', '{{$item->recibo}}')">
                                                    <i class="ri-information-line"></i>
                                                </button>
                                                @endif

                                                @if($item->estado_emissao == 'novo' || $item->estado_emissao == 'rejeitado')
                                                <a target="_blank" title="Visualizar XML Temporário" class="btn btn-light btn-sm text-dark" href="{{ route('mdfe.xml-temp', $item->id) }}">
                                                    <i class="ri-file-code-line"></i>
                                                </a>
                                                <button title="Transmitir MDF-e ao SEFAZ" type="button" class="btn btn-success btn-sm text-white" onclick="transmitir('{{$item->id}}')">
                                                    <i class="ri-send-plane-fill"></i>
                                                </button>
                                                @endif

                                                @if($item->estado_emissao == 'aprovado' || $item->estado_emissao == 'cancelado')
                                                <button title="Consultar Protocolo SEFAZ" type="button" class="btn btn-light btn-sm text-dark" onclick="consultar('{{$item->id}}', '{{$item->mdfe_numero}}')">
                                ...
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-4">
                                        ...
                                        Nenhum manifesto MDF-e encontrado para os filtros selecionados.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-3">
                    <div>
                        <span class="text-muted fs-12">Exibindo {{ $data->count() }} de {{ $data->total() }} manifestos</span>
                    </div>
                    <div>
    ...
    <div class="modal-dialog">
        <div class="modal-content text-dark">
            <div class="modal-header">
                <h5 class="modal-title" id="modalCancelarLabel">Cancelar MDF-e <strong class="ref-numero text-danger"></strong></h5>
...
<script type="text/javascript">
    function info(motivo_rejeicao, chave, estado, recibo) {
        if (estado == 'rejeitado') {
            ...
            swal("Manifesto Rejeitado", text, "warning");
        } else {
            let text = "Chave: " + chave + "\nRecibo: " + recibo + "\n";
            swal("Manifesto Autorizado", text, "success");
        }
    }
</script>

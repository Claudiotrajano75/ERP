@extends('layouts.app', ['title' => 'NF-e — Todas as Empresas'])
        ...
        <div class="card">
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        <h4 class="modulo-title text-white">
                            <i class="ri-file-text-line"></i> NF-e — Todas as Empresas
                        </h4>
                        <p class="modulo-subtitle">
                            Visão global e gerenciamento de todas as Notas Fiscais Eletrônicas emitidas no sistema.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body">
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h4 class="text-uppercase fs-12 mt-0 text-white-50">Total de NF-e</h4>
                                        <h3 class="my-1 text-white fs-20 fw-bold">{{ number_format($stats['total'] ?? 0, 0, ',', '.') }}</h3>
                                        <p class="mb-0 text-white-50 fs-11">Documentos gerados</p>
                                    </div>
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        ...
                                        <p class="mb-0 text-white-50 fs-11">Notas aprovadas</p>
                                    </div>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title bg-white bg-opacity-25 text-white rounded rounded-3 fs-3 widget-icon-box-avatar shadow">
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        ...
                                        <p class="mb-0 text-white-50 fs-11">Notas canceladas</p>
                                    </div>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title bg-white bg-opacity-25 text-white rounded rounded-3 fs-3 widget-icon-box-avatar shadow">
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between">
                                    <div class="flex-grow-1 overflow-hidden">
                                        ...
                                        <p class="mb-0 text-white-50 fs-11">Soma das notas autorizadas</p>
                                    </div>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title bg-white bg-opacity-25 text-white rounded rounded-3 fs-3 widget-icon-box-avatar shadow">
                ...
                <div class="col-lg-12 mb-3">
                    ...
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-md-3 col-12 d-flex gap-2">
                            ...
                            <a id="clear-filter" class="btn btn-danger px-3" href="{{ route('nfe-all') }}">
                        ...
                        <table class="table table-centered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-medium text-dark">{{ $item->cliente ? $item->cliente->razao_social : '--' }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="fs-12 text-muted">
                                            {{ $item->data_emissao ? __data_pt($item->data_emissao) : \Carbon\Carbon::parse($item->created_at)->format('d/m/Y H:i') }}
                                        </span>
                                    </td>
                                    <td class="text-end">
                                        <form action="{{ route('nfe.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="m-0">
                                            ...
                                            <div class="modulo-action-group">
                                                ...
                                                @if($item->estado == 'aprovado')
                                                <a class="btn btn-primary btn-sm text-white" target="_blank" href="{{ route('nfe.imprimir', [$item->id]) }}" title="Imprimir DANFE">
                                                    <i class="ri-printer-line"></i>
                                                </a>
                                                <button title="Cancelar NFe" type="button" class="btn btn-danger btn-sm text-white" onclick="cancelar('{{$item->id}}', '{{$item->numero}}')">
                                                    <i class="ri-close-circle-line"></i>
                                                </button>
                                                <button title="Carta de Correção (CC-e)" type="button" class="btn btn-warning btn-sm text-white" onclick="corrigir('{{$item->id}}', '{{$item->numero}}')">
                                                    <i class="ri-file-warning-line"></i>
                                                </button>
                                                @endif

                                                @if($item->estado == 'aprovado' || $item->estado == 'rejeitado')
                                                <button type="button" class="btn btn-dark btn-sm text-white" title="Ver Detalhes do Retorno SEFAZ" onclick="info('{{$item->motivo_rejeicao}}', '{{$item->chave}}', '{{$item->estado}}', '{{$item->recibo}}')">
                                                    <i class="ri-information-line"></i>
                                                </button>
                                                @endif

                                                @if($item->estado == 'novo' || $item->estado == 'rejeitado')
                                                <a target="_blank" title="Visualizar XML Temporário" class="btn btn-light btn-sm text-dark" href="{{ route('nfe.xml-temp', $item->id) }}">
                                ...
                                <tr>
                                    <td colspan="8" class="text-center text-muted py-4">
                                        ...
                                        Nenhuma nota fiscal eletrônica encontrada para os filtros selecionados.
                                    </td>
                                </tr>
                                @endforelse
                            </tbody>
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mt-3">
                    <div>
                        <span class="text-muted fs-12">Exibindo {{ $data->count() }} de {{ $data->total() }} notas fiscais</span>
                    </div>
                    <div>
    ...
    <div class="modal-dialog">
        <div class="modal-content text-dark">
            <div class="modal-header">
                <h5 class="modal-title" id="modalCancelarLabel">Cancelar NF-e <strong class="ref-numero text-danger"></strong></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row g-2">
                    <div class="col-12">
                        <label class="form-label required">Motivo do Cancelamento (mín. 15 caracteres)</label>
...
<div class="modal fade" id="modal-corrigir" tabindex="-1" aria-labelledby="modalCorrigirLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content text-dark">
            <div class="modal-header">
                <h5 class="modal-title" id="modalCorrigirLabel">Carta de Correção Eletrônica (CC-e) <strong class="ref-numero text-warning"></strong></h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row g-2">
                    <div class="col-12">
                        <label class="form-label required">Texto da Correção (mín. 15 caracteres)</label>
                        {!!Form::textarea('motivo-corrigir', '')->attrs(['class' => 'form-control', 'rows' => 3, 'placeholder' => 'Informe as alterações que devem constar na CC-e...'])->required()!!}
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-secondary btn-sm" data-bs-dismiss="modal">Fechar</button>
                <button type="button" id="btn-corrigir" class="btn btn-warning btn-sm text-white">Transmitir CC-e</button>
            </div>
        </div>
    </div>
</div>

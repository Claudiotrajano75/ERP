            ...
            <div class="card-body">
                ...
                <div class="clearfix">
                    <div class="float-start mb-3">
                        ...
                        <br>
                        @if($item->imagem)
                        <a href="{{ route('produtos.download-imagem-principal', [$item->id]) }}" class="btn btn-sm btn-outline-success mt-1">
                            <i class="ri-download-2-line align-middle me-1"></i> Baixar
                        </a>
                        @endif
                    </div>
                    <div class="float-end d-flex flex-column align-items-end gap-2">
                        <h4 class="m-0">{{ $item->nome }}</h4>
                        <a href="{{ route('produtos.download-zip', [$item->id]) }}" class="btn btn-success btn-sm">
                            <i class="ri-download-2-line align-middle me-1"></i> Baixar todas as imagens (ZIP)
                        </a>
                    </div>
                </div>
                {!!Form::open()
                    ...
                    <div class="col-md-3 col-12">
                        <div class="card">
                            ...
                            <div class="card-footer d-flex gap-1">
                                <a href="{{ route('produtos.download-imagem-galeria', [$i->id]) }}" class="btn btn-sm btn-success flex-grow-1">
                                    <i class="ri-download-2-line align-middle me-1"></i> Baixar
                                </a>
                                <form action="{{ route('produtos.destroy-image', $i->id) }}" method="post" id="form-{{$i->id}}">
                                    @method('delete')
                                    @csrf
                                    <button type="button" class="btn btn-sm btn-danger btn-delete w-100">Remover</button>
                                </form>
                            </div>
                        </div>
                    </div>

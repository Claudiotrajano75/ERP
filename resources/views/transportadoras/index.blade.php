                    ...
                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                        <div>
                            ...
                            <p class="text-muted mb-0 modulo-subtitle fs-13">
                                Cadastre empresas de frete e logística parceiras para vinculação em emissões fiscais e entregas.
                            </p>
                        </div>
                        <div>
                            @can('transportadoras_create')
                            <a href="{{ route('transportadoras.create') }}" class="dash-btn dash-btn-primary">
                                <i class="ri-add-circle-line"></i> Nova Transportadora
                            </a>
                            @endcan
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-4 col-12">
                            <div class="stat-card stat-indigo">
                                <div>
                                    <div class="stat-label">Transportadoras Cadastradas</div>
                                    <div class="stat-value mt-1">{{ $stats['total'] }}</div>
                                </div>
                                <div class="stat-icon"><i class="ri-truck-line"></i></div>
                            </div>
                        </div>
                        <div class="col-md-4 col-6">
                            <div class="stat-card stat-emerald">
                                <div>
                                    <div class="stat-label">Com Registro ANTT</div>
                                    <div class="stat-value mt-1">{{ $stats['total_antt'] }}</div>
                                </div>
                        ...
                        <div class="col-md-4 col-6">
                            <div class="stat-card stat-amber">
                                <div>
                                    <div class="stat-label">Cidades / Regiões Atendidas</div>
                                    <div class="stat-value mt-1">{{ $stats['total_cidades'] }}</div>
                                </div>
                    ...
                    <div class="modulo-glass-filter-premium mb-4">
                        <div class="filtro-premium-header">
                            <h5 class="filtro-premium-title">
                                <i class="ri-search-line"></i> Filtrar Transportadoras
                            </h5>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-5 col-12">
                                <label class="form-label"><i class="ri-truck-line"></i> Nome ou Razão Social</label>
                                {!!Form::text('razao_social', '')->attrs(['class' => 'form-control', 'placeholder' => 'Digite a razão social da transportadora...'])!!}
                            </div>
                            <div class="col-md-4 col-6">
                                <label class="form-label"><i class="ri-fingerprint-line"></i> CPF / CNPJ</label>
                                {!!Form::text('cpf_cnpj', '')->attrs(['class' => 'cpf_cnpj form-control', 'placeholder' => '00.000.000/0000-00', 'type' => 'tel'])!!}
                            </div>
                            <div class="col-md-3 col-12 ms-auto d-flex align-items-end">
                                <div class="d-flex gap-2 w-100">
                                    ...
                                    <a class="dash-btn dash-btn-light px-3" href="{{ route('transportadoras.index') }}" title="Limpar Filtros">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                <div class="transp-avatar">
                                                    <i class="ri-truck-line"></i>
                                                </div>
                                                <div>
                                                    <span class="fw-bold text-dark d-block fs-13">{{ $item->razao_social }}</span>
                                                    @if($item->nome_fantasia && $item->nome_fantasia != $item->razao_social)
                                                        <span class="text-muted fs-11">{{ $item->nome_fantasia }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="fw-semibold text-dark">{{ $item->cpf_cnpj }}</span>
                                        </td>
                                        <td>
                                            @if($item->antt)
                                                <span class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1 fs-11 fw-semibold">
                                                    ANTT: {{ $item->antt }}
                                                </span>
                                            @else
                                        ...
                                        <td>
                                            <span class="badge bg-light text-dark border px-2 py-1 fs-12">
                                                <i class="ri-map-pin-line text-muted me-1"></i> {{ $item->cidade ? $item->cidade->info : 'Não definida' }}
                                            </span>
                                        </td>
                                        <td>
                                            <span class="fw-semibold text-dark">{{ $item->telefone }}</span>
                                        </td>
                                        <td class="text-end">
                                            <form action="{{ route('transportadoras.destroy', $item->id) }}" method="post" id="form-{{$item->id}}" class="m-0">
                                                ...
                                                <div class="act-group">
                                                    ...
                                                    <a class="act-btn act-edit" href="{{ route('transportadoras.edit', [$item->id]) }}" title="Editar Transportadora">
                                                        <i class="ri-pencil-line"></i>
                                                    </a>
                                                    @endcan
                                                    @can('transportadoras_delete')
                                                    <button type="button" class="act-btn act-del btn-delete" title="Excluir Transportadora">
                                    ...
                                    <tr>
                                        <td colspan="7">
                                            <div class="modulo-empty">
                                                <i class="ri-truck-line"></i>
                                                <p>Nenhuma transportadora cadastrada até o momento.</p>
                                            </div>
                                        </td>
                                    </tr>
                                    @endforelse

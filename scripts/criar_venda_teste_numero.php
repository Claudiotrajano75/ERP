...
$numero = isset($argv[1]) ? (int)$argv[1] : 26;
$seq = isset($argv[2]) ? (int)$argv[2] : 44;

$existe = Nfe::where('empresa_id', 2)->where('numero', $numero)->exists();
if ($existe) {
    echo "ABORTADO: já existe NFe com número $numero\n";
    exit(1);
}
...
echo "NFe de teste criada: id={$nfe->id} numero=$numero serie=1 estado={$nfe->estado}\n";
